...
public:
    ...
    ENDPOINT_ASYNC("POST", "/api/v1/singin", GUserPoint)
    {
        ...
        ENDPOINT_ASYNC_INIT(GUserPoint)
            ...
            Action act() override
            {
                ...
                oatpp::String login = request->getHeader("login");
                oatpp::String password = request->getHeader("password");
    ...
    ENDPOINT_ASYNC_INIT(MusPoint)
        ...
        Action act() override 
        {
            ...
            if ((queryParams.get("selfName") != nullptr) + (queryParams.get("albumName") != nullptr) + (queryParams.get("artistName") != nullptr) + (queryParams.get("regex") != nullptr ) != 1)
                return _return(controller->createResponse(Status::CODE_400));
            ...
            if (queryParams.get("regex") != nullptr)
            {
                auto ret = createJSON((*getFacadeMap())[it->second]->getMusicByRegex(queryParams.get("regex")));
    ...
    ENDPOINT_INFO(MusPoint)
    {
        ...
        info->queryParams.add<oatpp::String>("regex");
        info->queryParams["regex"].required = false;
    ...
    ENDPOINT_INFO(plstPoint)
    {
     ...
     ENDPOINT_ASYNC("PATCH", "/api/v1/playlists/{id}/muscomps", patplstPoint)
    {
        ENDPOINT_ASYNC_INIT(patplstPoint)
            ...
            Action act() override
            {
                ...
                if (request->getPathVariable("id") != nullptr)
                {
                    ...
                    const Json::Value ids = root;
                    vector<int> iids;

                    for (int i = 0; i < ids.size(); i++ )
                    {
                        auto jid = ids[i];
                        iids.push_back(std::atoi(std::string(jid["id"].asString()).c_str()));
                    }
            ...
            Action act() override
            {
                ...
                if (request->getPathVariable("id") != nullptr && request->getPathVariable("musid") != nullptr)
                {
                    (*getFacadeMap())[it->second]->updatePlaylist(
                                                        ...
                                                        MusItem(std::atoi(std::string(request->getPathVariable("musid")).c_str()), string(), string(), string(), string(), string()));
                    return _return(controller->createResponse(Status::CODE_200));
